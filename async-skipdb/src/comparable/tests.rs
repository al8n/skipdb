...
async fn txn_commit_with_callback_in<S: AsyncSpawner, R>(sleep: impl Future<Output = R>) {
  let db: ComparableDB<u64, u64, S> = ComparableDB::new().await;
  let mut txn = db.write().await;
  for i in 0..40 {
...
async fn txn_commit_with_callback_tokio() {
  txn_commit_with_callback_in::<TokioSpawner, _>(tokio::time::sleep(Duration::from_millis(10)))
    .await;
}

#[async_std::test]
async fn txn_commit_with_callback_async_std() {
  txn_commit_with_callback_in::<AsyncStdSpawner, _>(async_std::task::sleep(Duration::from_millis(
    10,
  )))
  .await;
}
