...
use core::{future::Future, hash::BuildHasher, mem};
  ...
  H: BuildHasher + Default + Clone + Send + Sync + 'static,
{
  ...
  pub async fn write(&self) -> WriteTransaction<D, IndexMapManager<D::Key, D::Value, H>, S, H> {
    WriteTransaction {
      db: self.clone(),
...
impl<D: Database, S: AsyncSpawner, H: Clone + 'static> TransactionDB<D, S, H> {
  /// Create a new writable transaction with the given pending writes manager to store the pending writes.
  pub async fn write_by<W: PendingManager>(&self, backend: W) -> WriteTransaction<D, W, S, H> {
    WriteTransaction {
      db: self.clone(),
      read_ts: self.inner.orc.read_ts().await,
      size: 0,
...
impl<D: Database, S: AsyncSpawner, H: Default> TransactionDB<D, S, H> {
  /// Open the database with the given options.
...
impl<D: Database, S: AsyncSpawner, H> TransactionDB<D, S, H> {
  /// Open the database with the given options.
