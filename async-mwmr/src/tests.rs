...
pub async fn txn_simple<'a, D, W, S>(
  ...
  W: PendingManager<Key = D::Key, Value = D::Value> + Send + Sync + 'static,
  S: AsyncSpawner,
{
...
pub async fn txn_write_skew<'a, D: Database, W: PendingManager<Key = D::Key, Value = D::Value>, S>(
  opts: D::Options,
  backend: impl Fn() -> W,
