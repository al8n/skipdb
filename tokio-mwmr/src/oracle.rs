...
use wmark::{TokioCloser, TokioWaterMark};
  ...
  H: BuildHasher,
{
  ...
  #[inline]
  fn cleanup_committed_transactions(&mut self, detect_conflicts: bool, read_mark: &TokioWaterMark) {
    if !detect_conflicts {
    ...
    let max_read_ts = read_mark.done_until();
...
impl<H> Oracle<H> {
  ...
  pub fn new(
    ...
    Self {
      ...
      read_mark: TokioWaterMark::new(read_mark_name, closer.clone()),
      txn_mark: TokioWaterMark::new(txn_mark_name, closer.clone()),
      closer,
    }
  }
  ...
  pub(super) async fn read_ts(&self) -> u64 {
    let read_ts = {
      ...
      if let Err(e) = self.read_mark.begin(read_ts) {
        panic!("{e}");
      }
      read_ts
    };
